<!-- <script lang="ts">
  ...
  const fieldComponents: Record<FormField['type'], any> = {
    text: TextField,
    email: TextField,
    phone: TextField,
  ...
  onMount(() => {
    ...
    if (savedData) {
      try {
        ...
        formState.update((state) => ({ ...state, data: formData }));
      } catch (e) {
  ...
  function handleFieldChange(fieldName: string, value: any) {
    ...
    // Update store
    formActions.updateField(fieldName, value);
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (result.success) {
        ...
        formState.update((state) => ({ ...state, isSubmitted: true, isSubmitting: false }));
      ...
      dispatch('error', { error: error instanceof Error ? error.message : 'Submission failed' });
    } finally {
      isSubmitting = false;
    }
  }
  ...
  function handleReset() {
    ...
    errors = {};
    localStorage.removeItem(`form_${config.id}_data`);
    formState.update((state) => ({ ...state, data: {}, errors: {}, isSubmitted: false }));
    dispatch('reset');
  }
...
<script lang="ts">
  ...
  onMount(() => {
    ...
    // Sync local formData with store (will be empty unless auto_save is explicitly enabled)
    formData = get(formState).data;
  ...
  function handleFieldChange(fieldName: string, value: any) {
    ...
    // Auto-save (only when explicitly enabled)
    if (config.settings?.auto_save === true) {
      localStorage.setItem(`form_${config.id}_data`, JSON.stringify(formData));
    }
  ...
  function getFieldLayout() {
    const columns = config.layout?.columns || 1;
    const fieldOrder =
      config.layout?.fieldOrder || config.fields.map((f) => f.name);
  ...
  $: fieldLayout = getFieldLayout();
  $: progress =
    (config.fields.filter((f) => f.required && formData[f.name]).length /
...
<div class="w-full max-w-4xl mx-auto">
  ...
  >
    {#if fieldLayout.length === 1}
      <!-- Single column layout -->
      {#each fieldLayout[0] as field, index (field.id)}
        <div
    ...
    {:else}
      ...
      <div
        class="grid gap-6"
        style="grid-template-columns: repeat({fieldLayout.length}, 1fr);"
      >
        {#each fieldLayout as column, columnIndex}
          <div class="space-y-6">
            {#each column as field, fieldIndex (field.id)}
              <div
                class="animate-in fade-in duration-300"
                style="animation-delay: {fieldIndex * 50}ms"
              >
                <svelte:component
                  this={getFieldComponent(field)}
                  {field}
                  ...
                  {disabled}
                  on:change={(e) =>
                    handleFieldChange(field.name, e.detail.value)}
                  on:error={(e) => handleFieldError(field.name, e.detail.error)}
                />
              </div>
            {/each}
